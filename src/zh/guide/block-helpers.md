...
代码块助手代码使得调用通过传递上下文参数的代码块的迭代器或自定义函数成为可能。

## 基本代码块

出于演示目的，让我们先定义一个和没有代码块助手代码一样的代码块助手代码。
...
为了更好地说明语法，让我们定义另一个代码块助手代码，为包装的文本添加一些标记。
...
代码块助手代码的一个常见用法是使用它们来定义自定义迭代器。实际上，所有 Handlebars 内置助手代码都被定义为是常规的
Handlebars 帮助器。让我们看一下内置的 `each` 助手代码的工作方式。
...
代码块助手代码的另一个常见用例是计算条件表达式。与迭代器一样，Handlebars 内置有 `if` 和 `unless` 代码块助手代码。
...
Handlebars 通过 options 参数给代码块助手代码提供附加元数据。下文有更多示例。
...
与普通的助手代码一样，代码块助手代码可以接受可选的 Hash 作为其最终参数。让我们重新看看 `list` 帮助器并使我们可以将任意数
...
Hash 参数提供了一种强大的方法，可以为代码块助手代码提供许多可选参数，而无需使得这件事因为参数位置的问题而变得更复杂。

代码块助手代码还可以将局部变量注入其子模板。这对于添加不在上下文中的其他数据很有用。
